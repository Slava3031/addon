util.AddNetworkString("plankton")
util.AddNetworkString("ShowKot")
util.AddNetworkString("showiconleft") ---вот ета дон кароч дон сматри когда дон этот чеченец дон хватает рукой которая не правая дон вот эта дон создает шаблон дон для запроса на среврев дон понял дон?
util.AddNetworkString("showiconright") -- и вот этот тоже дон толко дон он для этого для рука который правая понял дон?
...
local function PlayImpactSound(ply)
    if IsValid(ply) then        
        ply:EmitSound("physics/body/body_medium_impact_soft4.wav", 75, 100, 5, CHAN_BODY)
    end
end
...
function Faking(ply)
	...
	if ply.fake == true then
		SavePlyInfo(ply)
		local godmodeCheck = ply:HasGodMode()
		...
		if IsValid(ply:GetNWEntity("DeathRagdoll")) then
			...
			if godmodeCheck then ply:GodEnable() end
			ply:SetSuppressPickupNotices(false)
	...
	function(ply, inflictor, attacker)
		...
		if ply.Attacker ~= nil and ply.Attacker ~= ply:Nick() then
			if bonenames[tostring(ply.LastHit)] ~= nil then
				ply:ChatPrint("Вы были убиты игроком " .. ply.Attacker .. " в " .. bonenames[tostring(ply.LastHit)] .. ".")
			else
				ply:ChatPrint("Вы были убиты игроком " .. ply.Attacker .. ".")
			end

			--print(bonenames[tostring(ply.LastHit)])
		else
			ply:ChatPrint("Ты мертв...")
		end
	end
)
...
hook.Add(
	...
	function(ply)
		if ply.fake then
			...
			if rag:GetFlexIDByName("blink") then rag:SetFlexWeight(rag:GetFlexIDByName("blink"), 1) end
...
hook.Add(
	...
	function(ent, dmginfo)
        local godmoechk
		if IsValid(RagdollOwner(ent)) then godmoechk = RagdollOwner(ent) elseif ent:IsPlayer() then godmoechk = ent end
		if godmoechk and godmoechk:HasGodMode() then return end
		...
		local bone = GetPhysicsBoneDamageInfo(ent, dmginfo)
		local hitgroup
		local isfall
		...
		if RagdollDamageBoneMul[hitgroup] then
			if RagdollOwner(ent) then
				...
				if hitgroup == HITGROUP_HEAD then
					...
					if dmginfo:GetDamageType() == 1 and dmginfo:GetDamage() > 6 and ent:GetVelocity():Length() > 500 then
						RagdollOwner(ent):ChatPrint("Твоя шея ломается!")
				...
				if hitgroup == HITGROUP_LEFTARM then
					...
					if dmginfo:GetDamageType() == 2 and dmginfo:GetDamage() > 10 and RagdollOwner(ent).LeftArm > 0.6 then
						RagdollOwner(ent):ChatPrint("Твоя левая рука хрустит!")
				...
				if hitgroup == HITGROUP_LEFTLEG then
					...
					if dmginfo:GetDamageType() == 2 and dmginfo:GetDamage() > 15 and RagdollOwner(ent).LeftLeg > 0.6 then
						RagdollOwner(ent):ChatPrint("Твоя левая нога хрустит!")
						ent:EmitSound("NPC_Barnacle.BreakNeck", 100, 200, 1, CHAN_ITEM)
				...
				if hitgroup == HITGROUP_RIGHTLEG then
					...
					if dmginfo:GetDamageType() == 2 and dmginfo:GetDamage() > 15 and RagdollOwner(ent).RightLeg > 0.6 then
						RagdollOwner(ent):ChatPrint("Твоя правая нога хрустит!")
						ent:EmitSound("NPC_Barnacle.BreakNeck", 100, 200, 1, CHAN_ITEM)
				...
				if hitgroup == HITGROUP_RIGHTARM then
					...
					if dmginfo:GetDamageType() == 2 and dmginfo:GetDamage() > 10 and RagdollOwner(ent).RightArm > 0.6 then
						RagdollOwner(ent):ChatPrint("Твоя правая рука хрустит!")
						ent:EmitSound("NPC_Barnacle.BreakNeck", 100, 200, 1, CHAN_ITEM)
				...
				if hitgroup == HITGROUP_CHEST then
					...
					--[[if dmginfo:GetDamageType() == 1 and ent:GetVelocity():Length() > 800 and RagdollOwner(ent).Organs["spine"] > 0 then
						...
						RagdollOwner(ent):ChatPrint("Твоя спина ломается!")
						ent:EmitSound("NPC_Barnacle.BreakNeck", 511, 200, 1, CHAN_ITEM)
						dmginfo:ScaleDamage(0.3)
					end--]]
			...
			if not dmginfo:IsDamageType(DMG_CRUSH + DMG_GENERIC) then
			    if ent:IsPlayer() then if ent:HasGodMode() then return end end
				if ent:IsPlayer() and ent.fake == false and ent:Alive() then
				...
				if ent:IsPlayer() or IsValid(RagdollOwner(ent)) then
					...
					if huy then
						if ply.Organs["brain"] ~= 0 then
							...
							if ply.Organs["brain"] == 0 then
								ply:ChatPrint("Твои мозги вышибли.")
							end
						end
					end
				end
						...
						--[[ply.Organs["spine"] = math.Clamp(ply.Organs["spine"] - dmginfo:GetDamage(), 0, 1)
							if ply.Organs["spine"] == 0 then
								if not ply.fake then
									...
									ply:ChatPrint("Твоя спина ломается!")
								end
							end--]]
						end
					end
		...
		if IsValid(RagdollOwner(ent)) then
			if dmginfo:GetDamageType() == DMG_CRUSH then
				...
				if dmginfo:GetAttacker():IsRagdoll() then
					...
					local crushdmg = dmginfo:GetDamage()
					if crushdmg >= 0.0040 then
					    crushdmg = math.max(1, crushdmg)
						local fallLength = ent:GetVelocity():Length()
						if fallLength > 600 then
						    crushdmg = crushdmg * fallLength / 20
						end
					    RagdollOwner(ent):SetHealth(RagdollOwner(ent):Health() - crushdmg)
						--print(crushdmg, "Текущее здоровье: "..RagdollOwner(ent):Health())
					end
				end
			elseif dmginfo:IsDamageType(DMG_BULLET+DMG_BUCKSHOT) then
				if RagdollDamageBoneMul[hitgroup] then
				    local damagemultiplier = RagdollDamageBoneMul[hitgroup]
					...
					RagdollOwner(ent):SetHealth(RagdollOwner(ent):Health() - damage)
					--print(RagdollOwner(ent):Health())
				end
			end
...
concommand.Add(
	...
	function(ply)
		...
		Встаём как терминатор несмотря на тяжёлую кровопотерю и дикую боль--]]
		timer.Create("faketimer" .. ply:EntIndex(), 2, 1, function() end)
		if ply:Alive() then
			Faking(ply)
			ply.fakeragdoll = ply:GetNWEntity("DeathRagdoll")
...
local CustomWeight = {
	["models/player/police_fem.mdl"] = 100,
	["models/player/police.mdl"] = 100,
	["models/player/zombie_fast.mdl"] = 70,
	...
	["models/player/skeleton.mdl"] = 70,
	["models/player/soldier_stripped.mdl"] = 80,
	["models/player/zombie_soldier.mdl"] = 80,
}
		...
		for i, ply in ipairs(player.GetAll()) do
			...
			if IsValid(rag) then
				...
				if RagdollOwner(rag):Alive() then
					if not RagdollOwner(rag).Otrub then
						if ply:KeyPressed(IN_JUMP) and table.Count(constraint.FindConstraints(ply:GetNWEntity("DeathRagdoll"), "Rope")) > 0 then
							...
							if Try > (7 * RopeCount) then
								if RopeCount > 1 then
									...
									ply:ChatPrint("You untied yourself.")
								end
			...
			if TwoHandedOrNo[ply.curweapon] then
				...
				phys:ComputeShadowControl(shadowparams) --if 2handed
					...
					if ply:KeyDown(IN_SPEED) and not RagdollOwner(rag).Otrub then
					...
					if ply:KeyDown(IN_WALK) and not RagdollOwner(rag).Otrub then
				...
				for i=1,3 do
					...
					if(trace.Hit and !trace.HitSky)then
						...
						if(IsValid(cons))then
							rag.ZacConsRH=cons
							net.Start("showiconright") -- вот эта дон тот самый ахахахаха сука да иди нахуй дон
							net.Send(ply)
							PlayImpactSound(ply)
						end
						break
				...
				--]]
							...
							shadowparams.secondstoarrive = 0.2
				...
				--]]
							...
							shadowparams.maxangular = 100
							shadowparams.pos = rag:GetPhysicsObjectNum(1):GetPos()
							shadowparams.secondstoarrive = 0.5
							rag:GetPhysicsObjectNum(0):Wake()
							rag:GetPhysicsObjectNum(0):ComputeShadowControl(shadowparams)--]]
						end
					end
		...
		for i, ply in ipairs(player.GetAll()) do
			...
			if not (ply.OrgansNextThink > CurTime()) then
				...
				if ply.Organs and ply:Alive() then
					...
					--print(ply.Blood)
					if ply.Blood and ply.Blood <= 2000 then
					    if ply:IsPlayer() then
						    ply:SetHealth(ply:Health() - 1000000)
						end
					end
...
hook.Add("PlayerDeath", "noicons", function( ply )
net.Start("hideiconright")
net.Send(ply)
net.Start("hideiconleft")
net.Send(ply)
local randomdeath = math.random(0,100)
