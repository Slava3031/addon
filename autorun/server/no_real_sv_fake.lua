...
-- Обновленная таблица соответствия между названиями брони и их энтити
local ArmorEntityMapping = {
    ["Heavy-Vest"] = {
        ent = "ent_jack_gmod_ezarmor_htorso",
        offset = {pos = Vector(5, 0, 0), ang = Angle(0, 90, 0)}, -- Настраивайте под конкретный тип
        color = {r = 255, g = 255, b = 255, a = 255}
    },
    ["Light-Vest"] = {
        ent = "ent_jack_gmod_ezarmor_ltorso",
        offset = {pos = Vector(0, 0, 5), ang = Angle(0, 45, 0)},
        color = {r = 255, g = 255, b = 255, a = 255}
    }
    
}

-- Таблица костей для привязки брони
local armor_bones = {
    ["head"] = "ValveBiped.Bip01_Head1",
    ["chest"] = "ValveBiped.Bip01_Spine2",
    ["torso"] = "ValveBiped.Bip01_Spine1",
    ["shoulder_l"] = "ValveBiped.Bip01_L_UpperArm",
    ["shoulder_r"] = "ValveBiped.Bip01_R_UpperArm",
    ["leg_l"] = "ValveBiped.Bip01_L_Thigh",
    ["leg_r"] = "ValveBiped.Bip01_R_Thigh"
}

-- Функция для создания брони на регдолле
local function CreateArmor(ragdoll, item)
    local armorClass = ArmorEntityMapping[item.name]
    if not armorClass then
        print("[ERROR] Armor mapping not found for:", item.name)
        return nil
    end

    -- Создание энтити брони
    ...
    -- Получение кости
    local boneName = armor_bones[item.type]
    local boneIndex = boneName and ragdoll:LookupBone(boneName)
    if not boneIndex then
        print("[WARNING] Bone not found for armor type:", item.type)
        return nil
    end

    -- Позиция и угол кости
    local bonePos, boneAng = ragdoll:GetBonePosition(boneIndex)
    if not bonePos then
        print("[ERROR] Failed to get bone position for:", boneName)
        return nil
    end

    -- Настройка позиции с учётом смещения
    local offset = item.offset or {pos = Vector(0, 0, 0), ang = Angle(0, 0, 0)}
    local right, forward, up = boneAng:Right(), boneAng:Forward(), boneAng:Up()

    local pos = bonePos
    pos = pos + right * offset.pos.x
    pos = pos + forward * offset.pos.y
    pos = pos + up * offset.pos.z

    local ang = boneAng
    ang:RotateAroundAxis(right, offset.ang.p)
    ang:RotateAroundAxis(up, offset.ang.y)
    ang:RotateAroundAxis(forward, offset.ang.r)

    -- Установка позиции и угла для брони
    ent:SetPos(pos)
    ent:SetAngles(ang)

    -- Настройка энтити
    ent.IsArmor = true
    ent:SetColor(Color(item.color.r, item.color.g, item.color.b, item.color.a))
    ent:Spawn()
    ...
    -- Привязка к кости регдолла
    constraint.Weld(ent, ragdoll, 0, ragdoll:TranslateBoneToPhysBone(boneIndex), 0, true, false)
...
local function TransferArmorToRagdoll(ply, ragdoll)
    ...
    for id, armor in pairs(ply.EZarmor.items) do
        ...
        -- Получаем данные о броне из маппинга
        local armorData = ArmorEntityMapping[armor.name]
        if not armorData or not armorData.ent then
            print("[ERROR] Armor class not found for:", armor.name)
            return
        end

        -- Создаём энтити брони
        local ent = ents.Create(armorData.ent)
        if not IsValid(ent) then
            print("[ERROR] Failed to create armor entity for:", armorData.ent)
            return
        end

        -- Настройка позиции и углов
        local boneName = armor_bones[armor.type]
        local boneIndex = boneName and ragdoll:LookupBone(boneName)
        if boneIndex then
            local bonePos, boneAng = ragdoll:GetBonePosition(boneIndex)

            -- Смещение позиции и угла
            local offset = armorData.offset or {pos = Vector(0, 0, 0), ang = Angle(0, 0, 0)}
            local right, forward, up = boneAng:Right(), boneAng:Forward(), boneAng:Up()
            ...
            -- Устанавливаем позицию и углы для энтити
            ent:SetPos(pos)
            ent:SetAngles(ang)
            ent:SetParent(ragdoll, boneIndex)
        else
            -- Если кость не найдена, ставим броню на тело
            ent:SetPos(ragdoll:GetPos())
            ent:SetParent(ragdoll)
        end

        ent:SetColor(Color(armorData.color.r, armorData.color.g, armorData.color.b, armorData.color.a))
        ent:Spawn()
    end
end


-- Обновляем Faking с новой логикой
function Faking(ply)
    ...
    if ply.fake then
        ...
        -- Восстановление игрока из фейка
        local ragdoll = ply:GetNWEntity("DeathRagdoll")
        if IsValid(ragdoll) then
...
-- Подключение дополнительного поведения для NPC
function FakeBullseyeTrigger(rag, owner)
    if not IsValid(rag.bull) then return end
    for _, npc in pairs(ents.GetAll()) do
        if npc:IsNPC() and npc:Disposition(owner) == D_HT then
            npc:AddEntityRelationship(rag.bull, D_HT, 0)
        end
    end
end
...
local function TransferArmorToRagdoll(ply, ragdoll)
    ...
    for id, armor in pairs(ply.EZarmor.items) do
        ...
        -- Получаем класс брони из маппинга
        local armorClass = ArmorEntityMapping[armor.name]
        if not armorClass then
            print("[ERROR] Armor class not found for:", armor.name)
            return
        end
        ...
        if boneIndex then
            ...
            -- Корректируем углы
            boneAng:RotateAroundAxis(boneAng:Right(), 0)  -- Подправьте углы для корректного позиционирования
